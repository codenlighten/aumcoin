...
extern Value getredeemscript(const Array& params, bool fHelp);
...
{ //  name                      function                 safe mode?
  //  ------------------------  -----------------------  ----------
    ...
    { "getredeemscript",        &getredeemscript,        false },
...
public:
    ...
    {
        // Boost 1.70+: get_io_service() deprecated, use get_executor().context() from lowest layer
        ip::tcp::resolver resolver(static_cast<asio::io_context&>(stream.lowest_layer().get_executor().context()));
...
{
    // Accept connection
    // Boost 1.70+: get_io_service() replaced with get_executor().context()
    AcceptedConnectionImpl<Protocol>* conn = new AcceptedConnectionImpl<Protocol>(
        static_cast<asio::io_context&>(acceptor->get_executor().context()), 
        context, 
        fUseSSL);
...
void ThreadRPCServer2(void* parg)
{
    ...
    // Boost 1.66+: ssl::context constructor no longer takes io_service
    ...
    ip::tcp::endpoint endpoint(bindAddress, GetArg("-rpcport", 9442));
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    asio::io_service io_service;
    ssl::context context(ssl::context::sslv23);  // Boost 1.66+: no longer takes io_service
    context.set_options(ssl::context::no_sslv2);
    asio::ssl::stream<asio::ip::tcp::socket> sslStream(io_service, context);
    SSLIOStreamDevice<asio::ip::tcp> d(sslStream, fUseSSL);
    iostreams::stream< SSLIOStreamDevice<asio::ip::tcp> > stream(d);
    if (!d.connect(GetArg("-rpcconnect", "127.0.0.1"), GetArg("-rpcport", "9442")))
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "addmultisigmldsaaddress" && n > 0) ConvertTo<boost::int64_t>(params[0]);
    if (strMethod == "addmultisigmldsaaddress" && n > 1) ConvertTo<Array>(params[1]);
