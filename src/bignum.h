...
/** Phase 2.2: Refactored for OpenSSL 3.x - composition instead of inheritance */
class CBigNum
{
public:
    BIGNUM* bn;  // OpenSSL 3.x: BIGNUM is opaque, use pointer instead of inheritance
    ...
    CBigNum(signed char n)      { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(short n)            { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(int n)              { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(long n)             { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); if (n >= 0) setulong(n); else setint64(n); }
    CBigNum(int64 n)            { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setint64(n); }
    CBigNum(unsigned char n)    { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setulong(n); }
    CBigNum(unsigned short n)   { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setulong(n); }
    CBigNum(unsigned int n)     { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setulong(n); }
    CBigNum(unsigned long n)    { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setulong(n); }
    CBigNum(uint64 n)           { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setuint64(n); }
    explicit CBigNum(uint256 n) { bn = BN_new(); if (!bn) throw bignum_error("BN_new failed"); setuint256(n); }
