...
const string strMessageMagic = "AumCoin Signed Message:\n";
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                // Verify signature
                // IMPORTANT: Use SIGHASH_ALL not 0, so OP_CHECKMLDSASIG gets correct nHashType
                if (!VerifySignature(txPrev, *this, i, fStrictPayToScriptHash, SIGHASH_ALL))
                {
                    // only during transition phase for P2SH: do not invoke anti-DoS code for
                    // potentially old clients relaying bad P2SH transactions
                    if (fStrictPayToScriptHash && VerifySignature(txPrev, *this, i, false, SIGHASH_ALL))
...
bool CTransaction::ClientConnectInputs()
{
    ...
    // Take over previous transactions' spent pointers
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Verify signature
            // IMPORTANT: Use SIGHASH_ALL not 0, so OP_CHECKMLDSASIG gets correct nHashType
            if (!VerifySignature(txPrev, *this, i, true, SIGHASH_ALL))
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        // Phase 5.2: AumTestNet - Quantum-Safe Test Network
        // Unique magic bytes for testnet P2P messages
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "AumCoin 30/Dec/2025 Satoshi Vision Restored - All OP_CODES Enabled - Post-Quantum Ready";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xa1, 0xd2, 0xe3, 0xf4 }; // AumCoin: unique network identifier
