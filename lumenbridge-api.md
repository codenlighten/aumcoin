...
The Embeddings API provides powerful text embedding generation capabilities using OpenAI's latest embedding models. Generate vector representations of text for semantic search, similarity matching, clustering, and more.
...
Generate embeddings for multiple texts efficiently (automatically batched).
...
Best Practice: Always use batch endpoint for multiple texts - much more efficient than individual requests.
22. Chunked Embeddings

POST /api/embeddings/chunked

Automatically split long text into chunks and embed each chunk with overlap.

Request Body:

{
  "text": "Very long text document...",
  "chunkSize": 1000,                   // characters per chunk (optional, default: 1000)
  "overlap": 100,                      // overlapping characters (optional, default: 100)
  "model": "text-embedding-3-small"    // optional
}
...
Use Case: Semantic search - find most relevant documents for a query.
25. Available Models

GET /api/embeddings/models

List all available embedding models with specifications.
  ...
  "models": [
    {
      ...
      "dimensions": 1536,
      "description": "Smaller, faster, cost-effective embedding model",
      "costPer1MTokens": 0.02
    },
    {
      "name": "text-embedding-3-large",
      "dimensions": 3072,
      "description": "Larger, more accurate embedding model",
...
26. Service Statistics

GET /api/embeddings/stats

View embeddings service usage statistics.
...

{
  ...
  "stats": {
    ...
    "totalEmbeddings": 156,
    "totalTokens": 3248,
    "averageTokensPerRequest": 77
  },
  "timestamp": "2025-12-31T12:00:00.000Z"
}

Reset Statistics: POST /api/embeddings/stats/reset
Embeddings Example: Semantic Search

// 1. Index documents
const docs = await fetch('/api/embeddings/batch', {
  method: 'POST',
...
const indexed = await docs.json();

// 2. Search with natural language
const query = await fetch('/api/embeddings/generate', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
    text: "How to learn AI and ML?"
  })
});

const queryEmbed = await query.json();

// 3. Find similar
const results = await fetch('/api/embeddings/find-similar', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({
...
const similar = await results.json();
console.log('Most relevant:', similar.results[0]);
